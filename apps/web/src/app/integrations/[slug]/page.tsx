...
export async function generateMetadata({
  ...
  return pageMetadata({
    ...
    // The card is generated by ./opengraph-image.tsx.
    image: null,
  });
}
            ...
            <div className="rounded-xl border border-ash-border bg-warm-sand">
              ...
              <div className="divide-y divide-ash-border">
                {data.reads.map((row) => (
                  <div
                    key={row.field}
                    className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 px-4 py-2.5 font-mono text-[12px]"
                  >
                    <span className="shrink-0 text-driftwood">{row.field}</span>
                    <span className="ml-auto min-w-0 text-right text-midnight-ink wrap-break-word">
                      {row.source}
                    </span>
                  </div>
                ))}
            ...
            <div className="rounded-2xl border border-ash-border bg-warm-sand p-6 sm:p-10">
              {data.cta === "crucible" ? (
                ...
                <>
                  ...
                  {/* Only a trace source supplies the traces; every other integration works on
                      whichever traces you already have. */}
                  <p className="mt-2 max-w-md text-body text-driftwood">
                    {data.reads.length > 0 ? (
                      <>
                        The CLI runs from npx, nothing to install; your{" "}
                        {data.name} traces do the rest.
                      </>
                    ) : (
                      <>
                        The CLI runs from npx, nothing to install; the traces
