...
// Each card renders the product's own vernacular (the 8 trace sources, the per-step
...
// Reveal only (static spine, no draw); AnimatedNumber is deliberately withheld —
  ...
  const steps: Step[] = [
    {
      ...
      sentence:
        "Point it at the traces you already emit. It autodetects the format across eight sources and folds every run into one per-step schema.",
      substance: (
        <div className="flex flex-col gap-y-2 font-mono text-[13px]">
          <Row label="reads">
            Claude Code · Codex · LangSmith / LangGraph · OpenAI SDK · Langfuse
            · Braintrust · Phoenix · OTel GenAI
          </Row>
          <Row label="folds">
            8 sources{arrow}
    ...
    },
    {
      ...
      title: "Replay & judge",
      sentence:
        "Shortlist cheaper OpenRouter models and re-run each step on your real inputs. Single-shot steps replay in isolation; tool and loop steps re-execute their code in a sandboxed git worktree, so cascades surface before you ship them.",
      substance: (
        <div className="flex flex-col gap-y-2 font-mono text-[13px]">
          <Row label="replay">
        ...
        <div className="font-mono text-[13px]">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-max whitespace-nowrap pt-2">
              ...
              <div className="px-1 py-1">
                <span className="text-fog">✕</span>
                {dot}auth_code_edit{dot}gpt-4.1{arrow}—{dot}—{dot}—{dot}
                <span className="text-fog">abstain · high-risk</span>
              </div>
            </div>
          </div>
          <p className="px-1 pt-3 text-caption text-fog">
            illustrative example — no savings figure until it runs on your
            traces
          </p>
        </div>
      ),
    ...
    <section id="how-it-works" className="bg-parchment-white text-midnight-ink">
      <div className="mx-auto max-w-4xl px-6 py-24 sm:px-10 sm:py-32">
        ...
        <Reveal className="max-w-xl">
          ...
          <h2 className="mt-4 text-balance font-display text-heading tracking-tighter text-midnight-ink sm:text-heading-lg">
            Four steps from your own traces to a swap you can defend.
          </h2>
          <p className="mt-4 text-body tracking-tighter text-driftwood">
            ...
            >
              ...
              <div className="grid grid-cols-[2.5rem_1fr] gap-x-4 sm:grid-cols-[3.5rem_1fr] sm:gap-x-6">
                ...
                <div className="rounded-xl border border-ash-border bg-warm-sand p-5 sm:p-6">
                  <h3 className="font-sans text-heading-sm tracking-tighter text-midnight-ink">
                    {step.title}
                  </h3>
                  <p className="mt-2 max-w-prose text-body tracking-tighter text-driftwood">
                    {step.sentence}
                  </p>
