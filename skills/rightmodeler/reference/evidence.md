...
`--catalog-reference` joins a model, its recency is the reference's release date, and the date
the gateway declares counts only when the reference gives none. Break equal sums by
